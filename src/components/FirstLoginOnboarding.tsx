...
function speak(text: string, onEnd?: () => void): () => void {
  ...
  const voice =
    voices.find((v) => /samantha|karen|daniel|alex|victoria|moira|fiona/i.test(v.name)) ??
...
export default function FirstLoginOnboarding({ onComplete }: Props) {
  ...
  const handleLetsStart = () => {
    ...
    cancelTtsRef.current = speak(INTRO_SCRIPT, () => {
      cancelTtsRef.current = speak(PROMPT_READY, () => {
  ...
  return (
    <AnimatePresence mode="wait">
      ...
      >
          <div className="flex flex-col items-center justify-center gap-10 px-6 max-w-lg text-center">
            {step === "idle" && (
              <>
                <motion.div
